...
function ProductDetails(){
    ...
    const images = [product.image1,product.image2,product.image3,product.image4,product.image5].filter(image => image); // ← دي بتشيل أي قيمة فاضية أو null أو undefined
    ...
    useEffect(() => {
      ...
      setProductVariantItem (variantProduct.find(
        (item) => item.product == product_id && item.size.id == selectedSize && item.color.id == selectedColor
      ))
      console.log("CartDictItem from useffect",productVariantItem);
     }, [variantProduct,selectedColor,selectedSize]);

    const handleAddToCart = (e) => {
      e.preventDefault();
      try{
        dispatch(addToCart(productVariantItem,currentUser,quantity))
        ...
        <>
          ...
          <div class={openModalContext?"modal-backdrop fade in":""}></div>
                      ...
                      Write a review
                          ...
                          <ul id="group_3">
                          {selectedSize?[...new Map(variantProduct.map(item => [item.color.id, item])).values()].map((item) => (
                          ...
                          disabled={ selectedColor!== nll? selectedColor!== item.color.id :false } 
